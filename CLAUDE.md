...
1. **スターターに既にあり、このリポジトリで前提として読むドキュメント**（現時点では主に参照・遵守。パスを列挙する）
   ...
   - `docs/universal/` 配下（例: `sdd-principles.md`, `question-document-spec.md`, `new-project-bootstrap.md`, `security-settings.md`, `skills-authoring.md`, `context-handoff.md`, `agent-workflow.md`, `error-workflow.md`）
...
#### ツール間の再読み込み（Cursor と Claude Code）

一方で編集した内容をもう一方が把握するには **自動では再読み込みされない**。タスク切替時は、読むパスを明示するプロンプトを使う。

**コピペ用の定型文と運用の説明**は **`docs/universal/context-handoff.md`** に置く。Human はそのファイルを参照し、必要なら `session-log` の Next に「相手ツール用プロンプトを貼る」とメモしてもよい。

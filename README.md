...
**推奨フロー:** 空ディレクトリで `git init` → スターターの**中身**をコピー → Claude Code に「まず質問して」と伝える → 質問ドキュメントに回答 →「回答を読んで実行して」。
...
`CLAUDE.md` の「新規プロジェクト開始時（質問ファースト）」により、テンプレートから質問ドキュメントが作成される。

テンプレや仕様書を明示したい場合の例（任意）：
